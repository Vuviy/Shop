      ...
      <div class="container">
        <div class="row">
          <section class="top-sales">
            ...
            $query = "SELECT `categorie` FROM `goods_categories` WHERE `id` = $categorie_id";
            $cat = mysqli_query($conn, $query);

            while ($c = mysqli_fetch_assoc($cat)) {
            ?>
              <h2 class="section-title cat-title"><?php echo $c['categorie'] ?></h2>
            <?php
            }
            ?>
            ...
            <div class="top-sales-cards">
              <?php
              $query_item = "SELECT * FROM `goods` WHERE `categorie_id` = $categorie_id";
              $items = mysqli_query($conn, $query_item);
              while ($item = mysqli_fetch_assoc($items)) {
              ?>
                <div class="card-item">
                  <div class="card-photo">
                    <a href="../../pages/goods.php?id=<?php echo $item['id']  ?>">
                      <img src="../../img/dress/<?php echo $item['photo'] ?>" alt="">
                    </a>
                  </div>
                  <div class="info-card">
                    <h2><?php echo mb_substr($item['title'], 0, 15) ?></h2>
                    <div class="info-info">
                      <span class="price">Ціна: <?php echo $item['price'] ?><span>грн</span></span>
                      <span class="sizes"><?php echo $item['size'] ?></span>
                    </div>
                    <a href="../../pages/goods.php?id=<?php echo $item['id']  ?>"><button class="buy-btn">купити</button></a>
                  </div>
                </div>
              <?php
              }
